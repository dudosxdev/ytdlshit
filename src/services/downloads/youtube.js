...
const createYouTubeDownloads = ({
  ...
  const processYouTubeDownloadAndCache = async (
    ...
    try {
      ...
      if (!document) throw new Error('Failed to get YT document after channel upload.');
      const fileId = serializeInputDocument(document);
      console.log(`${logPrefix} Extracted YT document: ${document.id}`);
  ...
  const processYouTubeDownloadRequestNormalWithCache = async (
    ...
    const sendFileToUser = async (fileId, videoDetails) => {
      ...
      try {
        ...
        console.error(`${logPrefix} Telegram send YT file_id error for ${youtubeId} (FileID: ${fileId}):`, errorDescription || telegramError);
