...
import { parseSerializedDocument } from '../gramjs/file-cache.js';
...
const registerInlineHandlers = ({
  ...
  bot.on('chosen_inline_result', async (ctx) => {
    ...
    if (directDownloadMatchYT) {
      ...
      if (cachedFileId && parseSerializedDocument(cachedFileId) && videoDetails) {
        ...
        if (cachedFileId && !parseSerializedDocument(cachedFileId)) {
          delete fileIdCacheStore.getCache()[cacheKey];
          await fileIdCacheStore.save().catch(() => {});
        }
    ...
    if (spotifyDownloadMatch) {
      ...
      const cachedFileId = fileIdCacheStore.getCache()[cacheKey];
      if (cachedFileId && parseSerializedDocument(cachedFileId) && metadata) {
