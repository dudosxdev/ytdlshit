...
import { createBot } from './gramjs/bot.js';
...
const startBot = async () => {
  ...
  bot.use(async (ctx, next) => {
    ...
    const updateId = ctx.update?.update_id || ctx.update?.queryId || ctx.update?.pts || 'N/A';
  ...
  const shutdown = async (signal) => {
    ...
    console.log('⏳ Stopping GramJS client (will happen on process exit)...');
    ...
    const botStopPromise = bot.stop().catch((error) => {
      console.error('⚠️ Error stopping GramJS client:', error);
      exitCode = 1;
    });
    ...
    await Promise.all([...cachePromises, expressClosePromise, dbClosePromise, botStopPromise]);
